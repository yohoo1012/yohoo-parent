...
import yo.hoo.core.utils.BaseDAO.DaoException.ExceptionId;
import yo.hoo.support.utils.GenericsUtils;
import yo.hoo.support.utils.StringUtils;
...
public abstract class BaseDAO<T> {
	...
	@Autowired
	private HibernateTemplate emrHibernateTemplate;
	...
	public void saveOrUpdate(T t) {
		try {
			emrHibernateTemplate.saveOrUpdate(t);
		} catch (Exception e) {
	...
	public void saveOrUpdateAll(Collection<T> entities) {
		try {
			emrHibernateTemplate.saveOrUpdateAll(entities);
		} catch (Exception e) {
			throw new DaoException(ExceptionId.保存时, e);
		}
	}
	...
	public void deleteAll(Collection<T> entities) {
		try {
			emrHibernateTemplate.deleteAll(entities);
		} catch (Exception e) {
			throw new DaoException(ExceptionId.删除时, e);
		}
	}
	...
	public T findById(Serializable id) {
		try {
			return emrHibernateTemplate.get(entityClass, id);
		} catch (Exception e) {
	...
	protected List<T> find(String hql, Object... values) {
		try {
			return emrHibernateTemplate.find(hql, values);
		} catch (Exception e) {
			throw new DaoException(ExceptionId.查询时, e);
		}
	}
	...
	protected Page pagedQuery(final String resultHql, String countHql, int pageNo, final int pageSize,
		...
		try {
			...
			List list = emrHibernateTemplate.executeFind(new HibernateCallback() {
